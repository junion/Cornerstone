...
class AgentThread(threading.Thread):
    def __init__(self, session_id, log_dir, in_queue, out_queue, result_queue):
        ...
        self.user_input_parser = importlib.import_module(domain)
    ...
    def run(self):
        while True:
            try:
                ...
                out_event = self.agent.run_by_output()
                
                # convert the cornerstone output to a galaxy frame
                self.actuator.execute(self.session_state, out_event)
...
#                email_log.send_mail('Exception!',traceback.format_exc())
    ...
    def _update_state_for_event(self, frame):
        ...
        if frame.name == 'begin_session':
            ...
            if event_type == 'user_utterance_end':
                ...
                self._session_state_change_handler(frame)
    ...
    def _system_utterance_end_handler(self, frame):
        ...
        if frame[':properties'][':bargein_pos'] != '-1':
            ...
            self.app_logger.info('Barge in detected at %s' % 
                                 frame[':properties'][':tagged_prompt'])
        if frame[':properties'][':utt_count'] in self.session_state.notify_prompts:
            self.session_state.notify_prompts.remove(frame[':properties'][':utt_count'])
    ...
    def _session_state_change_handler(self, frame):
